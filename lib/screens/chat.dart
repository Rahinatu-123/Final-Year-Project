...
class _ChatScreenState extends State<ChatScreen> {
  ...
  List<String> _chatParticipantsForWrite() {
    final participants = <String>{};
    if (userId != null && userId!.isNotEmpty) {
      participants.add(userId!);
    }

    // Most 1:1 chats use a deterministic id: "uidA_uidB".
    final idParts = widget.chatId
        .split('_')
        .where((id) => id.trim().isNotEmpty)
        .toList();
    if (idParts.length == 2) {
      participants.addAll(idParts);
    }
  ...
  Future<void> _ensureChatParticipants() async {
    final participants = _chatParticipantsForWrite();
    ...
    await FirebaseFirestore.instance.collection('chats').doc(widget.chatId).set(
      {
        ...
        'requestAcceptedAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
      },
      SetOptions(merge: true),
    );
  ...
  Future<void> _ignoreMessageRequest() async {
    await FirebaseFirestore.instance.collection('chats').doc(widget.chatId).set(
      {
        'requestStatus': 'rejected',
        'requestRejectedAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
      },
      SetOptions(merge: true),
    );
  ...
  Widget _buildMessageInput() {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final requestStatus = (chatData?['requestStatus'] ?? '').toString();
        final requestRecipientId = (chatData?['requestRecipientId'] ?? '')
            .toString();
        final isIncomingPendingRequest =
            requestStatus == 'pending' && requestRecipientId == userId;
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          'Accept to move this chat to your regular conversations.',
                          ...
                          children: [
                            Expanded(
                              child: OutlinedButton(
                                onPressed: _ignoreMessageRequest,
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .set({
            ...
            'updatedAt': FieldValue.serverTimestamp(),
            'participants': FieldValue.arrayUnion(_chatParticipantsForWrite()),
          }, SetOptions(merge: true));
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
